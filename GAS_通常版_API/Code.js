 ...
 *                        登録・確認済みである必要がある)
 *   SPREADSHEET_ID       注文台帳スプレッドシートのID(プロジェクトごとに個別設定。共通envには入れない)
 *
 * 共通envファイルに書くキー一覧:
...
function processOrderForm(formData) {
  ...
  var isExpress = (hasCorp && (formData.isExpress === 'true' || formData.isExpress === 'on'));
  var isDigital = (hasCorp && (formData.isDigital === 'true' || formData.isDigital === 'on'));

  // 納期（発送までの営業日数）の判定。基本納期（特急オプションの有無）に対し、
  // フリーメイト・ブラストチタンを含む場合はより日数のかかる方を採用する（まとめて発送するため）。
  var baseDeliveryDays = isExpress ? 3 : 7;
  var baseDeliveryLabel = isExpress ? "特急発送（3営業日以内）" : "通常発送（7営業日以内）";
  var hasTitanAny = (hasCorp && formData.corpMaterial === 'ブラストチタン') || _singleHasMaterial(formData, 'Titan');
  var deliveryType = _resolveDeliveryType(baseDeliveryDays, baseDeliveryLabel, hasFrei, hasTitanAny);
  ...
  if (hasCorp) {
    ...
    var corpPrice = (corpMaterial === '黒水牛') ? PRICE_CORP_KURO : (corpMaterial === 'ブラストチタン') ? PRICE_CORP_TITAN : tsugePrice;
    total += corpPrice;
  ...
  sendOrderEmails(formData, total, hasCorp, corpMaterial, innerTitle, hasFrei, freiTexts, singleResult, inkQty, isExpress, isDigital, deliveryType, fullAddress, corpFont, paymentUrl, sameAsAbove, shipZip, shipAddress, shipName, shipTel);
...
function sendOrderEmails(data, total, hasCorp, corpMaterial, innerTitle, hasFrei, freiTexts, singleResult, inkQty, isExpress, isDigital, deliveryType, fullAddress, corpFont, paymentUrl, sameAsAbove, shipZip, shipAddress, shipName, shipTel) {
  ...
  var slackHeader = isExpress
    ? "🚨🚨🚨 *【特急：最優先対応】* 🚨🚨🚨\n" + mention + " *特急注文が入りました！*"
    : mention + " *【印鑑注文が入りました】*";

  var slackText = slackHeader + "\n\n" +
                  "*■基本情報*\n" +
                  "・注文者: " + data.userName + " 様\n" +
