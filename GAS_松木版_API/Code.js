 ...
 *
 * GAS_通常版_APIとの違い: 松木様専用。14時までの注文で翌営業日発送を標準とし、
 * 法人3本セットの価格に特急発送分(2,200円)を最初から含めている(オプション化しない)。
 * 送料は通常版と同じ770円。
...
// 価格設定（税込・特急発送分2,200円を含んだ価格）
const PRICE_CORP_TSUGE_NORMAL = 11000;
const PRICE_CORP_KURO  = 24200;
const PRICE_CORP_TITAN = 95700; // 2026-08 法人3本セット価格改定(税込93,500円 + 特急発送分2,200円)
const FREIMATE_UNIT_PRICE = 1320;
const SHIPPING_FEE = 770;
...
function processOrderForm(formData) {
  ...
  // 納期（発送までの営業日数）の判定。基本納期は翌営業日発送（1営業日相当）。
  // フリーメイト（7営業日）・ブラストチタン（10営業日）・黒水牛（2営業日、松木版のみ適用）を
  // 含む場合は、より日数のかかる方を採用する（まとめて発送するため）。
  var hasTitanAny = (hasCorp && formData.corpMaterial === 'ブラストチタン') || _singleHasMaterial(formData, 'Titan');
  var hasKuroAny = (hasCorp && formData.corpMaterial === '黒水牛') || _singleHasMaterial(formData, 'Kuro');
  var deliveryType = _resolveDeliveryType(1, DELIVERY_TYPE, hasFrei, hasTitanAny, hasKuroAny);
  ...
  sendOrderEmails(formData, total, hasCorp, corpMaterial, innerTitle, hasFrei, freiTexts, singleResult, inkQty, isDigital, deliveryType, fullAddress, corpFont, paymentUrl, sameAsAbove, shipZip, shipAddress, shipName, shipTel);
...
function sendOrderEmails(data, total, hasCorp, corpMaterial, innerTitle, hasFrei, freiTexts, singleResult, inkQty, isDigital, deliveryType, fullAddress, corpFont, paymentUrl, sameAsAbove, shipZip, shipAddress, shipName, shipTel) {
  ...
  var slackText = mention + " *【印鑑注文（松木様専用）が入りました】*\n\n" +
                  "*■基本情報*\n" +
                  "・注文者: " + data.userName + " 様\n" +
